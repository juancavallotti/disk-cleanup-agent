...
- Do not measure or report the global size of the user's home directory—it is not useful and wastes time. Instead, inspect the sizes of possible problematic folders inside their home (or other common locations): use get_common_offender_paths and similar targets (caches, temp, trash, package-manager dirs, web_search). Focus on known problem areas; do not blindly drill down or measure everything.
...
2. **Execution phase:** Call get_system_type and get_current_username so the plan is OS-aware. Call get_common_offender_paths to get candidate directories (do not measure the whole home—target these candidates and other known problem folders). Call command_probe with relevant command names (e.g. npm, yarn, docker, brew). Then explore: list_folders and list_folder_contents_by_size on those candidate paths; use get_folder_capacity_batch to measure only the paths you are evaluating (not the home root). For each location that is safe to clean, call report_cleanup_opportunity with opportunities: [{ path, pathDescription, sizeBytes, contentsDescription, whySafeToDelete, suggestedAction?, recommendedCommand? }, ...]. When done, summarize and stop.
